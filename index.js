...
client.once("ready", () => {
  ...
  console.log("Code by hypexilegend");
});
...
client.on("messageCreate", async (message) => {
  ...
  if (!member.roles.cache.has(allowedRoleId)) {
    return message.reply({
      content: "**You Dont Have Whitlist To Use Me,**Sorry",
      ephemeral: true,
    });
  }
  ...
  if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
    return message.reply({
      content: "**You Dont Have Perm TO Use Me, **Sorry",
      ephemeral: true,
    });
  }
  ...
  const embed = new EmbedBuilder()
    .setColor("#0099ff")
    .setTitle("Brodcaster Bot Dashboard)
    .setImage(config.image)
    .setDescription("Please Chose The Type Of Brodcaste");
  ...
  try {
    ...
    if (interaction.isModalSubmit()) {
      ...
      await interaction.editReply({
        content: "Send To Selected Members ,**Done!**",
      });
    }
  } catch (error) {
